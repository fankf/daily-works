...
public class ClearDirTest {
    ...
    public static void main(String[] args) {
        final String mavenRepositoryPath = "D:\\workspace\\wgfin";
        File f = new File(mavenRepositoryPath);
        ClearDirTest clean = new ClearDirTest();
    ...
    public void cleanLastUpdate(File f) {
        if (f != null) {
            if (f.getName().endsWith("HEAD") || f.getName().endsWith("HEAD_0") ||
                    f.getName().endsWith("master") || f.getName().endsWith("master_0")
            ) {
                lastUpdateNum++;
